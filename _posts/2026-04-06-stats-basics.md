...
For those transitioning to mathematics / machine learning from other fields, sometimes the symbols and terminologies in statistics seem intuitive but lack rigour and confidence. These basic concepts will add confidence when reading mathematical texts and research papers dealing with probabilities. 
...
the limit of event \(A\)'s relative frequency if the experiment is repeated infinitely many times: 

$$ P(A) = \displaystyle \lim_{n \to \infty} \frac{\text{number of times A occurred in $n$ repetitions}}{n}$$
...
- For mutually exclusive events \(A_i \cap A_j = \emptyset\) for \( i \neq j\),
...
For a finite set \(A_1, A_2, A_3, ..., A_n\) of events, they are **mutually independent** iff for every \(k \leq n\) and every choice of \(A_{i_1}, ..., A_{i_k}\) of \(k\) events from \(A_1, A_2, A_3, ..., A_n\), 
